...
#define tableID4 @"SelectHeaderTableViewCell"

@interface EWashNextViewController ()<UITableViewDelegate,UITableViewDataSource,BagTableViewCellDelegate,LaundrySelectTableViewCellDelegate>
{
...
- (void)viewDidLoad {
    ...
    [ArrayTable addObject:@[FGGetStringWithKeyFromTable(@"0", @"Language"),FGGetStringWithKeyFromTable(@"1", @"Language"),FGGetStringWithKeyFromTable(@"2", @"Language")]];
...
-(void)addDowntableUI
{
    ...
    [DownTable registerNib:[UINib nibWithNibName:@"LaundrySelectTableViewCell" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:tableID3];
    [DownTable registerNib:[UINib nibWithNibName:@"SelectHeaderTableViewCell" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:tableID4];
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    }else if(indexPath.section==1)
    {
        if(indexPath.row==0)
        {
            ...
            SelectHeaderTableViewCell * cellB = (SelectHeaderTableViewCell *)[tableView dequeueReusableCellWithIdentifier:tableID4];
            if (cellB == nil) {
                cellB= (SelectHeaderTableViewCell *)[[[NSBundle  mainBundle]  loadNibNamed:@"SelectHeaderTableViewCell" owner:self options:nil]  lastObject];
            }
            //cell选中效果
            cellB.selectionStyle = UITableViewCellSelectionStyleNone;
            return cellB;
        }else{
        LaundrySelectTableViewCell * cellA = (LaundrySelectTableViewCell *)[tableView dequeueReusableCellWithIdentifier:tableID3];
        if (cellA == nil) {
            cellA= (LaundrySelectTableViewCell *)[[[NSBundle  mainBundle]  loadNibNamed:@"LaundrySelectTableViewCell" owner:self options:nil]  lastObject];
        }
        cellA.delegate=self;
        //cell选中效果
...
-(void)returnSelect:(NSInteger)selectFlag
{
    NSLog(@"selectFlag == %ld",(long)selectFlag);
    if(selectFlag==1)
    {
