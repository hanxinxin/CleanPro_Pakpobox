...
@interface DryerOrderViewController ()<UITableViewDelegate,UITableViewDataSource>
@property (nonatomic,strong)NSMutableArray * arr_title;
@end

@implementation DryerOrderViewController
...
-(void)getOrderList
{
    ...
    [[AFNetWrokingAssistant shareAssistant] GETWithCompleteURL_token:[NSString stringWithFormat:@"%@%@?page=%ld&size=%ld&siteTypeList=DRYER",E_FuWuQiUrl,E_WasherDryerquery,(long)self.page,(long)self.maxCount] parameters:nil progress:^(id progress) {
        
    } success:^(id responseObject) {
        NSLog(@"responseObject ORder1=  %@",responseObject);
