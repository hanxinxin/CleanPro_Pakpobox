...
-(void)setPassword
{
    ...
    //  背景色 方便  看
    self.password_text.backgroundColor = [UIColor whiteColor];
    //  距离
    self.password_text.elementMargin = 0;
    // 边框宽度
    self.password_text.elementBorderWidth = 1;
...
//                NSLog(@"NONONO");
                ...
                [HudViewFZ showMessageTitle:FGGetStringWithKeyFromTable(@"Two password inconsistencies", @"Language") andDelay:2.0];
            }
        }
    };
...
- (IBAction)Confirm_touch:(id)sender {
    ...
    [[AFNetWrokingAssistant shareAssistant] PostURL_Token:[NSString stringWithFormat:@"%@%@",FuWuQiUrl,P_XiugaiPayPassword] parameters:dict progress:^(id progress) {
        ...
        if(statusCode==200)
        {
            [HudViewFZ showMessageTitle:FGGetStringWithKeyFromTable(@"Modify successfully", @"Language") andDelay:2.0];
