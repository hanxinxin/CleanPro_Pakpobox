//
//  NewPasswordSetViewController.m
//  Cleanpro
//
...
@interface NewPasswordSetViewController ()<UIGestureRecognizerDelegate,UITextFieldDelegate>

@end

@implementation NewPasswordSetViewController
...
- (IBAction)Next_touch:(id)sender {
    ...
    [self.navigationController pushViewController:vc animated:YES];
         */
//        [self SkipInviteCode];  ////屏蔽CleanPro以前的注册
        [self registerUser];
    }
}
-(void)SkipInviteCode
...
//    };
    [[AFNetWrokingAssistant shareAssistant] PostURL_Code_error:[NSString stringWithFormat:@"%@%@",E_FuWuQiUrl,E_register] parameters:dict progress:^(id progress) {
        ...
        if(codeStr==nil)
        {
            ...
            //            [self.navigationController popToViewController:LoginViewController animated:YES];
            for (UIViewController *controller in self.navigationController.viewControllers) {
                if ([controller isKindOfClass:[NewLoginViewController class]]) {
                    [self.navigationController popToViewController:controller animated:YES];
                }
            }
