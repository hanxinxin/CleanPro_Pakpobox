...
@interface NewRegisterViewController ()<LMJDropdownMenuDelegate,UIGestureRecognizerDelegate,UITextFieldDelegate>
{
    int Time_cout;
}
...
- (IBAction)Next_touch:(id)sender {
    [self YZ_E_verifySmsCode];
}
-(void)push_NewPasswordView
{
...
//    [self.navigationController pushViewController:vc animated:YES];
        UIStoryboard *main=[UIStoryboard storyboardWithName:@"Main" bundle:nil];
        NewPasswordSetViewController *vc=[main instantiateViewControllerWithIdentifier:@"NewPasswordSetViewController"];
        vc.hidesBottomBarWhenPushed = YES;
        vc.Nextmode=self.Nextmode;
        vc.PhoneStr=self.phone_Textfield.text;
        vc.CodeStr=self.verification_textfield.text;
        [self.navigationController pushViewController:vc animated:YES];
//    }
}

-(void)YZ_E_verifySmsCode
{
    if(self.phone_Textfield.text.length>=8 && self.verification_textfield.text.length>0)
    {
        [HudViewFZ labelExample:self.view];
        NSDictionary * dict=@{@"mobile":self.phone_Textfield.text,
