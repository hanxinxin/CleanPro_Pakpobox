//
//  NewForgetSetPasswordViewController.m
//  Cleanpro
//
...
@interface NewForgetSetPasswordViewController ()<UIGestureRecognizerDelegate,UITextFieldDelegate>
{
...
@implementation NewForgetSetPasswordViewController
...
- (IBAction)Complete_touch:(id)sender {
    ...
    NSDictionary * dict=@{@"mobile":self.NewPhone,
                          @"newPassword":self.setPassword_textField.text,
                          @"validateCode":self.YZMStr,
    ...
    [[AFNetWrokingAssistant shareAssistant] PostURL_Code:[NSString stringWithFormat:@"%@%@",E_FuWuQiUrl,E_retrievePasswordReset] parameters:dict progress:^(id progress) {
        ...
        if(statusCode==200)
        {
            ...
            [HudViewFZ showMessageTitle:FGGetStringWithKeyFromTable(@"Set up successfully", @"Language") andDelay:2.0];
            ...
            for (UIViewController *temp in self.navigationController.viewControllers) {
                if ([temp isKindOfClass:[NewLoginViewController class]]) {
                    [self.navigationController popToViewController:temp animated:YES];
                    ////            return NO;//这里要设为NO，不是会返回两次。返回到主界面。
                }
            }
