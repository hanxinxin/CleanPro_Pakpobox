//
//  NewFirgetPasswordViewController.m
//  Cleanpro
//
...
#import "NewForgetSetPasswordViewController.h"

@interface NewForgetPasswordViewController ()<LMJDropdownMenuDelegate,UIGestureRecognizerDelegate,UITextFieldDelegate>
{
     int Time_cout;
}
...
//        }];
        ...
        //        NSLog(@"url=== %@",[NSString stringWithFormat:@"%@%@",FuWuQiUrl,P_sendVerifyCode]);
        [[AFNetWrokingAssistant shareAssistant] PostURL_Code:[NSString stringWithFormat:@"%@%@",E_FuWuQiUrl,E_retrievePassword] parameters:dict progress:^(id progress) {
            NSLog(@"请求成功 = %@",progress);
        }Success:^(NSInteger statusCode,id responseObject) {
            [HudViewFZ HiddenHud];
            if(statusCode==200)
...
//            NewForgetSetPasswordViewController *vc=[main instantiateViewControllerWithIdentifier:@"NewForgetSetPasswordViewController"];
...
//    }];
    NSDictionary * dict=@{@"mobile":self.phone_textfield.text,
                          @"validateCode":self.Verification_textfield.text};
    //        NSLog(@"url=== %@",[NSString stringWithFormat:@"%@%@",FuWuQiUrl,P_sendVerifyCode]);
    [[AFNetWrokingAssistant shareAssistant] PostURL_Code:[NSString stringWithFormat:@"%@%@",E_FuWuQiUrl,E_retrievePasswordVerify] parameters:dict progress:^(id progress) {
        NSLog(@"请求成功 = %@",progress);
    }Success:^(NSInteger statusCode,id responseObject) {
        NSLog(@"responseObject = %@",responseObject);
        [HudViewFZ HiddenHud];
        ...
        {
            NSDictionary * dictObject=(NSDictionary *)responseObject;
            NSString*tokenStr = [dictObject objectForKey:@"userToken"];
            
            UIStoryboard *main=[UIStoryboard storyboardWithName:@"Main" bundle:nil];
            NewForgetSetPasswordViewController *vc=[main instantiateViewControllerWithIdentifier:@"NewForgetSetPasswordViewController"];
            vc.TokenString=tokenStr;
            vc.YZMStr=self.Verification_textfield.text;
            vc.NewPhone= self.phone_textfield.text;
            vc.hidesBottomBarWhenPushed = YES;
            [self.navigationController pushViewController:vc animated:YES];
        }else
        {
...
//            self.NextStep.backgroundColor=[UIColor colorWithRed:172/255.0 green:220/255.0 blue:251/255.0 alpha:1.0];
        ...
        if (range.length == 1 && string.length == 0) {
            ...
            self.Verification_textfield.text = [[textField.text stringByAppendingString:string] substringToIndex:4];
            dispatch_time_t delayTime = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.03/*延迟执行时间*/ * NSEC_PER_SEC));
