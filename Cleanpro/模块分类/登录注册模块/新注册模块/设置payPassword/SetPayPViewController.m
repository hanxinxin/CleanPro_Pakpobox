...
@interface SetPayPViewController ()<UIGestureRecognizerDelegate,UITextFieldDelegate>

@end

@implementation SetPayPViewController
...
//    self.next_btn.layer.borderColor = [UIColor colorWithRed:51/255.0 green:51/255.0 blue:51/255.0 alpha:1].CGColor;//设置边框颜色
    ...
    [self.title_text setText:FGGetStringWithKeyFromTable(@"Please set 6 digits password", @"Language")];
...
{
    __block SetPayPViewController *  weakSelf = self;
...
//            [self pushViewcontroller:password];
//        }else
//        {
...
- (IBAction)next_touch:(id)sender {
    if(self.pay_textfeld.text.length>0)
    {
        ...
        if(![self.PayOldPassWordStr isEqualToString:self.PayPassWordStr])
        {
            [self pushViewcontroller:self.PayPassWordStr];
        }else{
            [HudViewFZ showMessageTitle:FGGetStringWithKeyFromTable(@"Cannot be the same as the old password", @"Language") andDelay:2.5];
        }
    }
...
-(void)pushViewcontroller:(NSString *)strOne
{
    UIStoryboard *main=[UIStoryboard storyboardWithName:@"Main" bundle:nil];
    SetPayPTwoViewController *vc=[main instantiateViewControllerWithIdentifier:@"SetPayPTwoViewController"];
    vc.hidesBottomBarWhenPushed = YES;
    vc.payStringOne=strOne;
    vc.PayOldPassWordStr=self.PayOldPassWordStr;
    vc.validateCode=self.validateCode;
    vc.index=self.index;
