...
- (instancetype)initWithCount:(NSInteger)count margin:(CGFloat)margin payanskip:(NSInteger)PayAndSkip
{
    if (self = [super init]) {
        
        self.itemCount = count;
        self.itemMargin = margin;
        self.PayAndSkip = PayAndSkip;
        [self configTextField];
    }
    return self;
}
...
- (void)configTextField
{
    ...
    for (NSInteger i = 0; i < self.itemCount; i++)
    {
        ...
        line.backgroundColor = [UIColor colorWithRed:163/255.0 green:163/255.0 blue:163/255.0 alpha:1];
