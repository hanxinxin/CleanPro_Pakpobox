...
@interface NewLoginViewController ()<LMJDropdownMenuDelegate,UIGestureRecognizerDelegate,UITextFieldDelegate>
...
//    self.SignIn_btn.backgroundColor=[UIColor colorWithRed:48/255.0 green:209/255.0 blue:250/255.0 alpha:1];
    ...
    {
        self.phone_textfiled.text=UserName;
    }else{
...
//    [self.navigationController pushViewController:vc animated:YES];
    ...
    UIStoryboard *main=[UIStoryboard storyboardWithName:@"Main" bundle:nil];
       NewForgetPasswordViewController *vc=[main instantiateViewControllerWithIdentifier:@"NewForgetPasswordViewController"];
       vc.hidesBottomBarWhenPushed = YES;
       [self.navigationController pushViewController:vc animated:YES];
}
...
-(void)NewSigin_log
{
        ...
        [HudViewFZ labelExample:self.view];
        NSString * tokenStr = [[NSUserDefaults standardUserDefaults] objectForKey:@"YHToken"];
        NSDictionary * dict=@{@"username":self.phone_textfiled.text,
                              @"password":self.verification_textfiled.text,
                              @"clientType":@"IOS",///客户端类型 IOS：水果、ANDROID：安卓
                              @"notificationToken":tokenStr,
        };
        NSLog(@"dict=== %@    url === %@",dict,[NSString stringWithFormat:@"%@%@",E_FuWuQiUrl,E_login]);
        [[AFNetWrokingAssistant shareAssistant] PostURL_Code_error:[NSString stringWithFormat:@"%@%@",E_FuWuQiUrl,E_login] parameters:dict progress:^(id progress) {
            NSLog(@"请求成功 = %@",progress);
        }Success:^(NSInteger statusCode,id responseObject) {
            [HudViewFZ HiddenHud];
            NSLog(@"responseObject = %@",responseObject);
            if(statusCode==200)
            {
                NSDictionary * dictObject=(NSDictionary *)responseObject;
                NSString*tokenStr = [dictObject objectForKey:@"token"];
                NSString*mobileStr = [dictObject objectForKey:@"mobile"];
                NSString*memberIdStr = [dictObject objectForKey:@"memberId"];
                NSString*nickname = [dictObject objectForKey:@"nickname"];
                NSString*username = [dictObject objectForKey:@"username"];
                NSString*userId = [dictObject objectForKey:@"userId"];
                NSNumber * statusCode2 =[dictObject objectForKey:@"errorCode"];
                NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
                if([statusCode2 intValue] ==400001)
                {
                    NSString*errorMessage = [dictObject objectForKey:@"errorMessage"];
                    [HudViewFZ showMessageTitle:errorMessage andDelay:2.0];
                }if([statusCode2 intValue] ==400002)
                {
                    [HudViewFZ showMessageTitle:FGGetStringWithKeyFromTable(@"Password error", @"Language") andDelay:2.0];
                }else if([statusCode2 intValue] == -1)
                {
                    
                    NSString*errorMessage = [dictObject objectForKey:@"message"];
                    [HudViewFZ showMessageTitle:errorMessage andDelay:2.0];
                }else if(statusCode2==nil){
                  
                   
                    
                    if(![memberIdStr isEqual:[NSNull null]])
                    {
                        [userDefaults setObject:memberIdStr forKey:@"memberId"];
                        [userDefaults setObject:mobileStr forKey:@"mobile"];
                    ...
                    NSNotification *notification =[NSNotification notificationWithName:@"tongzhi_UpdateTabbar" object: nil];
                                       //通过通知中心发送通知
                    [[NSNotificationCenter defaultCenter] postNotification:notification];
                    for (UIViewController *temp in self.navigationController.viewControllers) {
                    ...
                    for (UIViewController *temp in self.navigationController.viewControllers) {
                        if ([temp isKindOfClass:[IMessageViewController class]]) {
                            [self.navigationController popToViewController:temp animated:YES];
        ...
        } failure:^(id receive, NSError *error) {
                NSLog(@"error = %@",error);
//                [HudViewFZ showMessageTitle:FGGetStringWithKeyFromTable(@"Get error", @"Language") andDelay:2.0];
